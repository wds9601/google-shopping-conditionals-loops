...
// let productArray = data.items.filter((item) => {
//     return item.kind === 'shopping#product'
// })
// console.log(productArray.length)


// 2.) Print the `title` of all items with a `backorder` availability
// in `inventories`.

console.log('\nProblem 2:\n')

let backorderArray = data.items.inventories.filter((item) => {
    return item.availability === 'backorder'
    // let backorderArray = inventoryArray.filter
})
